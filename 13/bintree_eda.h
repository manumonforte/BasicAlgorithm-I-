	...
	T findLess(){
		if (this->right().empty() && this->left().empty())
			...
			if (!this->left().empty() && this->right().empty()){
				return std::min(this->left().findLess(), this->root());
			}
			else if (!this->right().empty() && this->left().empty()){
				return std::min(this->right().findLess(), this->root());
			}
			else {
				T aux = std::min(this->right().findLess(), this->left().findLess());
				return std::min(aux, this->root());
			}
		}
	}
